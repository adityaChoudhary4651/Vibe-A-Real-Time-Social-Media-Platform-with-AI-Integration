    ...
    <div className={`min-h-screen w-full max-w-full transition-colors duration-300 ${
      ...
      <div className="flex w-full h-full">
        ...
        <main className={`flex-1 w-full max-w-full ${
          isDiscoverPage || isCreatePage ? "h-screen overflow-hidden flex flex-col" : "min-h-screen"
        }`}>
          {/* Hide header on reels, messages, and discover (full-screen) */}
          {!isReelsPage && !isMessagesPage && !isDiscoverPage && <MobileHeader />}
          <div className={isReelsPage || isDiscoverPage || isCreatePage ? "h-full flex-1 min-h-0" : "pb-20 lg:pb-8"}>
            <Outlet context={outletContext} />
          </div>
        </main>

        {/* Right sidebar for larger screens */}
        {location.pathname !== "/search" && location.pathname !== "/discover" && location.pathname !== "/create" && (
